<template>
  <div class="max-w-4xl mx-auto space-y-6 p-4 sm:p-6 lg:p-8 dark:text-gray-200 pb-24 md:pb-8">
    <!-- Header Card -->
    <div class="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
      ...
      <p class="text-gray-600 dark:text-gray-400 mt-2 text-center">{{ $t('inspection.newDescription') }}</p>
    </div>
        ...
        <div v-if="item.status !== 'ok'" class="mt-4 space-y-4">
          ...
          <div>
            ...
            <div v-if="item.photos.length > 0" class="mt-4">
              ...
              <img 
                :src="item.photos[0].file_content" 
                :alt="$t('inspection.photoPreview')" 
                @click="openLightBox(item.photos[0].file_content)"
                class="max-w-xs h-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-600 cursor-pointer hover:shadow-xl transition-shadow"
              >
            </div>
          </div>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
      ...
      <div class="flex justify-end mt-4">
        <button @click="clearSignature" class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-600 rounded-md hover:bg-gray-300 dark:hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-transform active:scale-95">
    ...
    <!-- Submission -->
    <div class="fixed bottom-0 left-0 right-0 p-4 bg-slate-50/80 dark:bg-gray-900/80 backdrop-blur-sm border-t border-gray-200 dark:border-gray-700 md:relative md:p-0 md:bg-transparent md:dark:bg-transparent md:border-t-0">
      <div class="flex justify-center">
        <button @click="submitInspection" class="w-full max-w-md px-8 py-4 text-lg font-semibold text-white bg-primary-600 rounded-lg hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 shadow-lg transform hover:scale-105 transition-transform duration-200 active:scale-95">
    ...
    <!-- Lightbox -->
    <LightBox v-model:visible="isLightBoxVisible" :image-url="lightBoxImageUrl" />
  </div>
</template>
...
const fetchItems = async () => {
  try {
    ...
    // Initialize inspectionDetails based on fetched items ONLY if not already loaded from draft
    if (inspectionDetails.value.length === 0) {
...
const handlePhotoUpload = (event: Event, index: number) => {
  ...
  if (target.files && target.files[0]) {
    ...
    if (!file.type.startsWith('image/')) {
      showSnackbar(t('snackbar.selectAnImageFile'), 'error');
    ...
    new Compressor(file, {
      ...
      error(err) {
        ...
        showSnackbar(t('snackbar.imageCompressionFailed'), 'error');
        // Clear the photo array on failure
        inspectionDetails.value[index].photos = [];
        // Reset the file input
        target.value = '';
      },
...
const submitInspection = async () => {
  ...
  try {
    ...
    router.push('/records'); // Redirect to their records page
  } catch (error: any) {
...
onMounted(() => {
  const savedDraft = localStorage.getItem('inspection_draft');
  if (savedDraft) {
    if (confirm(t('confirm.loadDraft'))) {
      try {
        const parsedDraft = JSON.parse(savedDraft);
        // Basic validation to ensure draft is not malformed
        if (Array.isArray(parsedDraft) && parsedDraft.length > 0 && parsedDraft[0].item_id) {
          inspectionDetails.value = parsedDraft;
           // After loading the draft, also fetch the latest items to ensure item names are displayed correctly
          fetchItems();
        } else {
          // If draft is invalid, remove it and fetch items normally
          localStorage.removeItem('inspection_draft');
          fetchItems();
        }
      } catch (e) {
        console.error("Failed to parse inspection draft:", e);
        localStorage.removeItem('inspection_draft');
        fetchItems();
      }
    } else {
      // User chose not to load the draft, so remove it
      localStorage.removeItem('inspection_draft');
      fetchItems();
    }
  } else {
...
watch(inspectionDetails, (newDetails) => {
  // Don't save the initial empty state right after fetching items.
  // We can check if the form is "dirty" or just save on any change.
  // For simplicity, we save on any change, but not if it's empty.
  if (newDetails.length > 0 && newDetails.some(d => d.status !== 'ok' || d.comment || d.photos.length > 0)) {
    localStorage.setItem('inspection_draft', JSON.stringify(newDetails));
  }
}, { deep: true });
</script>